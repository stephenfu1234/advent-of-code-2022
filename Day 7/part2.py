...
def solve(filename):
    ...
    total_used_space = sum(filesystem['/'])
    current_unused_space = total_disk_space - total_used_space
    space_to_clear = required_unused_space - current_unused_space

    # find all the directories we can delete that will free up the required space    
    res = [filesizes for dir, filesizes in filesystem.items()]
    directory_sizes = list(map(sum, res))
    valid_dirs_to_delete = [dir for dir in directory_sizes if dir >= space_to_clear]
    
    # find the smallest directory we can delete that contains required_space_to_clear
    print(min(valid_dirs_to_delete))
