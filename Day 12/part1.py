...
def solve(filename):
    ...
    start_location = get_location_of_value('S', maze_with_border)
    target_location = get_location_of_value('E', maze_with_border)
    print('start location', start_location, 'target location', target_location)
        
    distances = network_graph.shortest_distances(start_location)
    #print(distances, "\n")

    print(distances[target_location])

    # to_F = distances["?F"]

    # print(f"The shortes?t distance from S to E is {to_F}")
    ...
    for row in range(0, len(maze)):
        for col in range(0, len(maze[0])):
            ...
            for neighbour_row, neighbour_col in directions:
                ...
                if get_elevation(maze[row][col]) + 1 >= get_elevation(maze[neighbour_row][neighbour_col]):
